...
def run_query(query):
    with conn.cursor() as cur:
        cur.execute(query)
        return cur.fetchall()
query_cau_6 = ("Select tmp_diem_thi_tong_hop.SBD, tmp_diem_thi_tong_hop.Ho, tmp_diem_thi_tong_hop.Ten, tmp_diem_thi_tong_hop.Phai, tmp_diem_thi_tong_hop.ngay_sinh, Toan, Van, anh_van, tong_diem, "
                            ...
                            +"where tong_diem >= (SELECT MAX(tong_diem) FROM tmp_diem_thi_tong_hop) order by xep_loai DESC, tong_diem DESC; ")
def cau6():
    result = run_query(query_cau_6)
    lst_SBD = []
    lst_Ho = []
    lst_Ten = []
    lst_phai = []
